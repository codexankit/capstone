...
const DocumentUploadPage = () => {
  ...
  // for storing the data from backend uploads
  ...
  const handleProofTypeChange = (e) => { 
    setDocumentData((prev) => ({ ...prev, addressProofType: e.target.value })); 
  };
  ...
  const uploadDocuments = async (appNumber, custId) => { 
    const formData = new FormData(); 
    formData.append("applicationNumber", appNumber); 
    formData.append("customerId", custId); 
    formData.append("addressProofType", documentData.addressProofType); 
    formData.append("addressProof", documentData.addressProof); 
    formData.append("panProof", documentData.panProof); 
    formData.append("incomeProof", documentData.incomeProof); 
    try { 
      ...
      console.error(err); 
      toast({ title: "Failed to upload documents", status: "error", duration: 2000 }); 
      return null; 
    } 
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast({ title: "Application & Documents submitted", status: "success", duration: 2000 }); 
 ...
 /* const handleSubmit = () => {
  ...
  const previewUrls = useMemo(() => {
    ...
    return {
      addressProof: make(documentData.addressProof),
      pan: make(documentData.pan),      
      income: make(documentData.income),
    };
  }, [files]);

  const allFilesChosen = (documentData.addressProof && documentData.pan && documentData.income);
      ...
      <Container maxW="6xl" pb={10}>
        <Box maxW="5xl" mx="auto" bg="white" rounded="xl" boxShadow="xl" p={{ base: 6, md: 8 }}>
          ...
          <FormControl isRequired>
            ...
            <Select value={documentData.addressProofType} onChange={handleProofTypeChange}>
